...
struct per_session_data__avrspi {
	int fd;
};
...
static int callback_avrspi(struct libwebsocket_context *context,
                         struct libwebsocket *wsi,
                         enum libwebsocket_callback_reasons reason, void *user,
                         void *in, size_t len)
{
    case LWS_CALLBACK_HTTP: {
        void *universal_response = "Hello, World1!";
        libwebsocket_write(wsi, universal_response, strlen(universal_response), LWS_WRITE_HTTP);
        break;
    }   
}
...
static struct libwebsocket_protocols protocols[] = {
	...
	},
	{
		"avrspi-protocol",
		callback_avrspi,
		sizeof(struct per_session_data__avrspi),
		10,
	},
...
int main(int argc, char **argv)
{
	...
	int option;
	while ((option = getopt(argc, argv,"bp:a:l:")) != -1) {
		switch (option)  {
	...
	/* Create socket to listen */
	    context = libwebsocket_create_context(sport, NULL, protocols,
